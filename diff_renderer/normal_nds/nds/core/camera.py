...
class Camera:
    ...
    def camera_with_angle(cls, scale, center, view_angle=0,
        ...
        if orthographic:
            ...
            K[2, 2] = 0.001  # 2 / (zFar - zNear), following ICON rendering src : zFar=100, zNear=-100
        else:
            K = np.identity(3)
            K[0, 0] = cam_params['fx']
            K[1, 1] = cam_params['fy']
    ...
    @classmethod
    def perspective_camera_with_angle(cls, view_angle=0, pitch=0,
                                      orthographic=False, cam_params=None,
                                      device='cpu'):
        R_delta = cls.make_rotate(math.radians(pitch), 0, 0)
        y = np.deg2rad(int(view_angle))
        R_np = np.array([[np.cos(y), 0, np.sin(y)],
                      [0, 1, 0],
                      [-np.sin(y), 0, np.cos(y)]])
