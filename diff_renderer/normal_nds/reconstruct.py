...
import matplotlib.pyplot as plt
import trimesh
from diff_renderer.normal_nds.utils import batch_rigid_transform, diff_forward_skinning, pose_extraction
...
class NormalNDS(nn.Module):
    ...
    def set_initial_mesh(self, smpl_mesh, smpl_infos, smpl_models, posed_meshes=None, canon_mesh=None, config=None):
        ...
        if config.canonfusion.use_initial_mesh and canon_mesh is not None:
            initial_mesh = mesh2open3d(canon_mesh, texture=None)
        else:
            initial_mesh = mesh2open3d(smpl_mesh, texture=None)
            
        self.smpl_mesh = smpl_mesh # for lbs assigning on forward skinning
        ...
        self.initial_mesh = Mesh(vertices=torch.tensor(np.asarray(initial_mesh.vertices)).contiguous(),
                                 indices=torch.tensor(np.asarray(initial_mesh.triangles)).contiguous(),
                                 colors=torch.zeros((np.asarray(initial_mesh.vertices).shape)).contiguous(),
                                 device=self.device)
    ...
    def forward(self, output_dir):
        ...
        for iteration in progress_bar:
            ...
            if iteration in self.upsample_iterations and (mesh.indices.shape[0]) <= 150000:
                ...
                initial_mesh = Mesh(mesh.vertices, mesh.faces, np.ascontiguousarray(np.zeros((mesh.vertices.shape))), device=self.device)
            ...
            if self.tpose:
                gbuffers = self.renderer.render(views_subset,
                                                deformed_mesh,
                                                channels=['mask', 'normal', 'color'],
